...
async def gamble(ctx,bet:float=0.0): 
	...
	if authorString not in bankdict.keys(): #create a new user in dict if not in dict
		bankdict.update({authorString:[100,366]}) #first number in the list is the default amount given to a new account, change as you see fit
	...
	for i in range(0,9):
		if display[i]==listt[time.localtime()[6]]:
			score+=1
	points=math.ceil((score/2.0)*bet) #average score is 2.22, the default value should cause a slight growth in money over time
...
async def balance(ctx,freebie:int=0):
	...
	if bankdict[authorString][1]!=time.localtime()[7]:
		freebie=25.0 #This is the number added daily, change as you see fit for balance
